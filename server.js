...
import {
	buildCaptureListboxScript,
	buildCaptureKebabMenuScript,
} from "./src/cdp-scripts/capture-dropdown.js";
...
// When TUNNEL_URL is set, AG2R is behind a reverse proxy (nginx, Cloudflare tunnel, etc.)
// that handles TLS termination — so the backend runs in HTTP-only mode.
const HTTP_ONLY = !!TUNNEL_URL;
...
const appName =
	_env === "production"
		? "AG2R"
		: `AG2R ${_env.charAt(0).toUpperCase() + _env.slice(1)}`;
const appIconPath =
	_env !== "production" &&
	fs.existsSync(path.join(__dirname, "public", `ag2r-icon-${_env}.png`))
		? `/ag2r-icon-${_env}.png`
		: "/ag2r-icon.png";
...
async function sendPushToAll(payload) {
	...
	log(
		"Push",
		`Sending to ${pushSubscriptions.size} subscriber(s): ${payload.body}`,
	);
	const stale = [];
	let sent = 0;
	for (const [endpoint, sub] of pushSubscriptions) {
		...
		try {
			...
			if (err.statusCode === 410) {
				...
				log(
					"Push",
					`✗ ${err.statusCode || "N/A"} — keeping subscription (${err.body || err.message})`,
				);
			}
		}
	}
...
function checkAttentionState(snapshot) {
	...
	const attentionItems = (snapshot.sidebarAttentionItems || []).filter(
	...
	const newItems = attentionItems.filter(
		(item) => !notifiedConversations.has(item.id),
	);
	if (newItems.length > 0) {
		console.debug(
			"[Push] New attention items:",
			newItems.map((i) => `${i.name}(${i.type})`).join(", "),
		);
	}
...
function ensureCerts() {
	if (HTTP_ONLY) return null; // No TLS needed — reverse proxy handles it
...
async function connectCDP() {
	...
	log(
		"CDP",
		`Connecting to "${discovery.target.title}" on port ${discovery.port}`,
	);
	...
	log(
		"CDP",
		`Connected. ${cdpContexts.length} execution context(s) available.`,
	);
	broadcastStatus();
	return client;
}
	...
	for (const ctx of sorted) {
		try {
			...
			if (result.exceptionDetails) {
				console.debug(
					"[CDP] Eval exception in context",
					ctx.id,
					result.exceptionDetails.text,
					JSON.stringify(result.exceptionDetails.exception || {}).substring(
						0,
						200,
					),
				);
...
async function captureSnapshot() {
	try {
		...
		try {
			result.runningTasksHtml =
				await evaluateAcrossContexts(RUNNING_TASKS_SCRIPT);
		} catch (e) {
		...
		try {
			result.scheduledTasksHtml = await evaluateAcrossContexts(
		...
		try {
			result.conversationHistoryHtml = await evaluateAcrossContexts(
		...
		if (result.scheduledTasksHtml) {
			try {
				result.scheduledTasksDialogHtml = await evaluateAcrossContexts(
					SCHEDULED_TASKS_DIALOG_SCRIPT,
				);
			} catch (e) {
				console.debug(
					"[Snapshot] Scheduled tasks dialog eval failed:",
					e.message,
				);
			}
			...
			if (!result.dropdownHtml) {
				try {
					...
					console.debug(
						"[Snapshot] Scheduled tasks dropdown eval failed:",
						e.message,
					);
				}
			}
			...
			if (!result.dropdownHtml) {
				try {
					...
					console.debug(
						"[Snapshot] Kebab context menu eval failed:",
						e.message,
					);
				}
			}
...
async function waitForEditorImage(maxWaitMs = 3000) {
	...
	for (let i = 0; i < attempts; i++) {
		try {
			...
			/* ignore eval errors during polling */
		}
	...
	async function poll() {
		...
		try {
			...
			if (snapshot) {
				...
				if (hash !== lastSnapshotHash) {
					console.debug(
						"[SidebarMirror:server] isSidebarOpen:",
...
app.use((req, res, next) => {
	...
	if (
		PUBLIC_PATHS.some((p) => req.path === p) ||
...
app.post("/copy-response", async (req, res) => {
	...
	if (!clickId || !cdpClient) {
		return res
			.status(400)
			.json({ error: "Missing clickId or CDP not connected" });
	}
	try {
		...
		log(
			"CopyResponse",
			`clickId=${clickId} text=${(result?.text || "").length} chars`,
		);
		res.json(result || { ok: false });
	} catch (e) {
	...
	try {
		...
		try {
			...
			for (const line of psOutput.split("\n")) {
				if (
					line.includes("Antigravity.app/Contents/MacOS/Antigravity") &&
					!line.includes("grep")
				) {
		...
		setTimeout(() => {
			...
			exec(
				`env -i HOME=${home} /bin/bash -l -c 'open -a Antigravity --args --remote-debugging-port=9000'`,
				(err) => {
...
app.post("/click", async (req, res) => {
	...
	try {
		...
		if (String(clickId).startsWith("subinfo:")) {
			const clickScript = buildMainClickScript(
				JSON.stringify(String(clickId)),
...
app.post("/submit-dialog", async (req, res) => {
	...
	log(
		"SubmitDialog",
		`clickId=${clickId} label="${label}" text="${text || ""}"`,
	);
...
app.post("/eval", async (req, res) => {
	try {
		log(
			"Eval",
			`Executing script: ${String(req.body.script).substring(0, 150)}...`,
		);
...
app.post("/clear-editor", async (req, res) => {
	try {
		const ctxId = await findEditorContext();
		if (!ctxId)
			return res.status(503).json({ error: "No editor context found" });
...
app.post("/type-text", async (req, res) => {
	...
	if (!placeholder && !clickId) {
		return res
			.status(400)
			.json({ error: "placeholder or clickId is required" });
	}
	if (!cdpClient) {
		return res.status(503).json({ error: "CDP not connected" });
	}
	...
	const typeScript = buildTypeTextScript(
		safePlaceholder,
		safeClickId,
		safeText,
	);
...
app.post("/upload", upload.single("image"), async (req, res) => {
	...
	log(
		"Upload",
		`Received ${fileName} (${mimetype}, ${(buffer.length / 1024).toFixed(1)}KB)`,
	);
	...
	try {
		...
		if (!result?.ok) {
			return res
				.status(500)
				.json({ error: result?.reason || "Injection failed" });
		}
...
app.post("/send", async (req, res) => {
	...
	log(
		"Send",
		`Received: "${message?.substring(0, 50)}"${hasImages ? " (with images)" : ""}${hasMacro ? " (with macro)" : ""}`,
	);
...
app.get("/push/status", (req, res) => {
	res.json({
		...
		endpoints: [...pushSubscriptions.keys()].map(
			(ep) => ep.substring(0, 80) + "...",
		),
...
app.get("/icon-workshop", (req, res) => {
	...
	if (!fs.existsSync(toolPath))
		return res.status(404).send("Icon workshop not found");
			...
			for (const part of parts) {
				if (part.includes('name="icon"')) {
					...
					const body = Buffer.from(
						part.substring(headerEnd + 4).replace(/\r\n$/, ""),
						"binary",
					);
					fs.writeFileSync(
						path.join(__dirname, "public", "ag2r-icon.png"),
						body,
					);
					return res.json({ ok: true });
				}
...
async function start() {
	...
	// Generate/load SSL certs (returns null when behind a reverse proxy)
	const sslOpts = ensureCerts();

	// HTTP or HTTPS based on whether TLS is terminated by a reverse proxy
	const server = HTTP_ONLY
		? createHttpServer(app)
		: createHttpsServer(sslOpts, app);
